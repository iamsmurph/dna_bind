...
def process_atom_features(
    ...
    for token_id, token in enumerate(data.tokens):
        ...
        rd_atoms = list(mol.GetAtoms())
        # Prefer name-based mapping when RDKit atoms store a 'name' property; otherwise fall back to index order
        if all(a.HasProp("name") for a in rd_atoms):
            atom_name_to_ref = {a.GetProp("name"): a for a in rd_atoms}
        else:
            atom_name_to_ref = None
        ...
        if atom_name_to_ref is not None:
            # Use name-based lookup; if any name missing, fall back to index order
            token_atoms_ref_list = []
            missing = False
            for a in token_atoms:
                ref = atom_name_to_ref.get(a["name"])  # type: ignore[index]
                if ref is None:
                    missing = True
                    break
                token_atoms_ref_list.append(ref)
            if missing:
                # Fallback: align by index within the residue
                if len(rd_atoms) >= token["atom_num"]:
                    token_atoms_ref_list = [
                        rd_atoms[i] for i in range(token["atom_num"])
                    ]
                else:
                    raise KeyError(
                        f"RDKit molecule for {token['res_name']} lacks atom names and has fewer atoms than expected"
                    )
            token_atoms_ref = np.array(token_atoms_ref_list, dtype=object)
        else:
            # No 'name' properties present: align by index
            if len(rd_atoms) >= token["atom_num"]:
                token_atoms_ref = np.array(
                    [rd_atoms[i] for i in range(token["atom_num"])], dtype=object
                )
            else:
                raise KeyError(
                    f"RDKit molecule for {token['res_name']} lacks atom names and has fewer atoms than expected"
