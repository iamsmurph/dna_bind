...
class BoltzWriter(BasePredictionWriter):
    ...
    def __init__(
        ...
        write_contact_probs: bool = False,
        write_tm_expected_value: bool = False,
        ...
        self.write_contact_probs = write_contact_probs
        self.write_tm_expected_value = write_tm_expected_value
        self.write_cropped_embeddings = write_cropped_embeddings
    ...
    def write_on_batch_end(
        ...
        for record, coord, pad_mask in zip(records, coords, pad_masks):
            ...
            for model_idx in range(coord.shape[0]):
                ...
                if self.write_tm_expected_value and "tm_expected_value" in prediction:
                    tm_expected_value = prediction["tm_expected_value"][model_idx]
                    path = (
                        struct_dir
                        / f"tm_expected_value_{record.id}_model_{idx_to_rank[model_idx]}.npz"
                    )
                    np.savez_compressed(
                        path,
                        tm_expected_value=tm_expected_value.cpu()
                        .float()
                        .numpy()
                        .squeeze(),
                    )
            ...
            if self.write_cropped_embeddings and "cropped_embeddings" in prediction:
                cropped_embeddings = prediction["cropped_embeddings"]
                path = struct_dir / f"cropped_embeddings_{record.id}.pt"
                torch.save(cropped_embeddings, path)

            if self.write_contact_probs and "contact_probs" in prediction:
                contact_probs = prediction["contact_probs"]
                path = struct_dir / f"contact_probs_{record.id}.npz"
                np.savez_compressed(
                    path,
                    contact_probs=contact_probs.cpu()
                    .float()
                    .to(torch.float16)
                    .numpy()
                    .squeeze(),
